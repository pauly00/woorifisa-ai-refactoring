...
/*
	accounting.js 파일을 리팩토링하려고 해. ES5 문법들을 ES6로 최신화 시켜줘. 
*/
...
(function(root, undefined) {
	...
	const checkCurrencyFormat = (format) => {
		const defaultFormat = lib.settings.currency.format;

		if (typeof format === "function") format = format();

		if (isString(format) && format.indexOf("%v") > -1) {
			return {
				pos : format,
				neg : format.replace("-", "").replace("%v", "-%v"),
				zero : format
			};
		} else if (!format || !format.pos || format.pos.indexOf("%v") < 0) {
			return (!isString(defaultFormat)) ? defaultFormat : lib.settings.currency.format = {
				pos : defaultFormat,
	...
	const unformat = lib.unformat = lib.parse = (value, decimal) => {
		if (Array.isArray(value)) {
			return value.map((val) => unformat(val, decimal));
		}
